...
#wait if game is running and start hopping when it is
    def check_if_csgo_running():
        if "csgo.exe" in (p.name() for p in psutil.process_iter()) is True:
            ...
            #change console name to random str:
            random_str2 = secrets.token_urlsafe()
            ctypes.windll.kernel32.SetConsoleTitleW(random_str2)


            time.sleep(1)
            os.system("cls")
            print(f"{Fore.RED}START CSGO...")
            check_if_csgo_running()
